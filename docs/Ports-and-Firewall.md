...
| **51940** | UDP | Inbound | Any | Mesh tunnel (boringtun) |
...
> **9101/tcp** does NOT need a firewall rule — it binds to the mesh tunnel IP (10.64.x.x) and is only reachable over the encrypted tunnel.
...
iptables -A INPUT -p udp --dport 51940 -j ACCEPT  # Mesh tunnel (boringtun)
...
/ip firewall nat add chain=dstnat action=dst-nat protocol=udp dst-address=67.204.56.242 dst-port=51940 to-addresses=10.10.12.16 to-ports=51940 comment="FRS-LMND-NXS-MESH"
...
ufw allow 51940/udp  # Mesh tunnel (boringtun)
