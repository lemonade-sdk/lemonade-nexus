...
TEST(SecurityCodec, LengthFieldsAreBoundedBeforeAllocation) {
    ...
    ASSERT_FALSE(bytes.empty());
    constexpr std::size_t kEnvelopeHeader = 1 + 2 + 2 + 2 + 32 + 8 + 32 + 4;
    // challenge_digest, node_id, incarnation, epoch, both rulesets, the profile
    // id and its ruleset, vote key.
    constexpr std::size_t kEvidenceFixed = 32 + 32 + 8 + 8 + 2 + 2 + 2 + 2 + 32;
