...
int run_first_run(const ServerConfig& config) {
    ...
    std::printf("\n");
    std::printf("  Every value above is REQUIRED; the server exits if root-pubkey,\n");
    std::printf("  genesis-pubkey or release-signing-pubkey is missing.\n");
    ...
    std::printf("                             mesh does not start, and a normal server\n");
    std::printf("                             start is refused.\n");
